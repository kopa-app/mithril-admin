...
module.exports = {
  ...
  controller: function (context, opts) {
    ...
    scope.collapsed = opts.collapsed || false;
  ...
  view: function (scope, context, opts) {
    ...
    function fieldView(name) {
      ...
      var childContext = _.merge(_.clone(context), {
        item: component.isNested ? context.item[context.fieldName] : context.item,
        fieldName: name
      });

      return m('.m-admin-field',
        ['relation', 'hidden', 'group'].indexOf(field.component) !== -1 ?
    ...
    return m((context.action === 'edit' ? 'fieldset' : '') + '.m-admin-group', [
      ...
      scope.collapsed ? null : Object.keys(opts.fields).map(fieldView)
    ]);
  }
};
