...
var c = module.exports = {
  ...
  controller: function (context, opts) {
    ...
    scope.normalizeOpts = function(opts) {
      ...
      } else if (opts.relationType === 'hasMany' || opts.relationType === 'hasOne') {
    ...
    scope.loadSingle = function () {
      ...
      if (opts.relationType === 'belongsTo') {
        query = context.item[context.fieldName];
      }
      else if (opts.relationType === 'hasOne') {
        query = {};
        query[opts.foreignKey] = context.resource.id(context.item);
      }
      ...
      scope.resource.load(query)
        .then(function (result) {
          util.arrReplace(scope.items, result.data ? (isArray(result.data) ? result.data : [result.data]) : []);
          scope.loading = false;
          hasLoaded = true;
        });
    };
